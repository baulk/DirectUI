...
DEFINE_DP_WITH_FLAGS(TextBlock, Text, std::wstring, L"",
...
DEFINE_DP_WITH_FLAGS(TextBlock, TextAlignment, DirectWrite::TextAlignment, DirectWrite::TextAlignment::Leading,
                     PropertyMetadataFlags::AffectsRender);
DEFINE_DP_WITH_FLAGS(TextBlock, ReadingDirection, DirectWrite::ReadingDirection,
                     DirectWrite::ReadingDirection::LeftToRight, PropertyMetadataFlags::AffectsRender);
...
void TextBlock::Measure(const DX::SizeF &maximumSize) {
  ...
  _format = factory.CreateTextFormat(FontFamily().c_str(), FontWeight(), FontStyle(),
                                     DX::DirectWrite::FontStretch::Normal, FontSize());
  _format.SetReadingDirection(ReadingDirection());

  _layout = factory.CreateTextLayout(Text().c_str(), static_cast<int>(Text().size()), _format, maximumSize.Width,
                                     maximumSize.Height);
