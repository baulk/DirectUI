...
template <typename T, typename... Types> static Ref<T> Create(Types &&...args) {
  return std::make_shared<T>(std::forward<Types>(args)...);
}

template <typename T> static Ref<T> Create(std::initializer_list<Ref<UIElement>> list) {
  return std::make_shared<T>(list);
}

class UIElement;

enum class BindingMode { Default, OneWay, TwoWay, OneTime };
...
public:
  ...
  template <typename T> const T &GetValue(const DependencyProperty<T> &dp) const { return dp.GetValue(*this); }

  template <typename TObject> TObject &As() const { return static_cast<TObject &>(*this); }

  template <typename TObject> const TObject *DynamicAs() const { return dynamic_cast<const TObject *const>(this); }
  ...
  virtual void SetWindow(Window *window) { _window = window; }

  template <typename T>
  void BindTo(DependencyProperty<T> &targetDP, DependencyObject &source, DependencyProperty<T> &sourceDP,
...
public:
  ...
  DependencyProperty(const char *name, const T &defaultValue = T(),
                     PropertyMetadataFlags flags = PropertyMetadataFlags::None)
      : _name(name), _defaultValue(defaultValue), _flags(flags) {}

  const std::string &GetName() const { return _name; }

  PropertyMetadataFlags GetFlags() const { return _flags; }

  const T &GetDefaultValue() const { return _defaultValue; }

  template <typename Type> Type &SetValue(Type &object, const T &value, bool checkForChange = true) {
    if (checkForChange) {
  ...
  void BindTo(DependencyObject &target, DependencyObject &source, DependencyProperty<T> &sourceDP,
              BindingMode mode = BindingMode::Default) {
    sourceDP.RegisterPropertyChangedHandler(
        &source,
        [&target, this](auto &object, auto &, const T &, const T &newValue) { target.SetValue(*this, newValue); });
  }
...
public:                                                                                                                \
  ...
  class *name(const type &value) {                                                                                     \
    return SetValueRef<type, class>(name##Property, value);                                                            \
  }                                                                                                                    \
...
public:                                                                                                                \
  static DependencyProperty<type> name##Property;                                                                      \
  class *name(const type &value) {                                                                                     \
    return SetValue<type, class>(name##Property, value);                                                               \
  }                                                                                                                    \
  const type &name() const { return GetValue(name##Property); }
...
#define DEFINE_DP(class, name, type, defaultValue) DependencyProperty<type> class ::name##Property(#name, defaultValue)
...
#define DECLARE_AP(name, type)                                                                                         \
  static DependencyProperty<type> name##Property;                                                                      \
  static void name(UIElement &element, type value) { element.SetValue(name##Property, value); }                        \
  static type name(UIElement &element) { return element.GetValue(name##Property); }                                    \
  static void name(Ref<UIElement> element, type value) { element->SetValue(name##Property, value); }

#define DEFINE_AP(class, name, type, defaultValue) DependencyProperty<type> class ::name##Property(#name, defaultValue);
